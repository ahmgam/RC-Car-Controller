...
{
	GPIO_WriteBit(LEFT_MOTOR_GPIO,LEFT_MOTOR_FORWARD_PIN,1);
	GPIO_WriteBit(LEFT_MOTOR_GPIO,LEFT_MOTOR_BACKWARD_PIN,0);

}
static void LeftMotorBackward(void)
{
	GPIO_WriteBit(LEFT_MOTOR_GPIO,LEFT_MOTOR_FORWARD_PIN,0);
	GPIO_WriteBit(LEFT_MOTOR_GPIO,LEFT_MOTOR_BACKWARD_PIN,1);
}

static void RightMotorForward(void)
{
	GPIO_WriteBit(RIGHT_MOTOR_GPIO,RIGHT_MOTOR_FORWARD_PIN,1);
	GPIO_WriteBit(RIGHT_MOTOR_GPIO,RIGHT_MOTOR_BACKWARD_PIN,0);
}
static void RightMotorBackward(void)
{
	GPIO_WriteBit(RIGHT_MOTOR_GPIO,RIGHT_MOTOR_FORWARD_PIN,0);
	GPIO_WriteBit(RIGHT_MOTOR_GPIO,RIGHT_MOTOR_BACKWARD_PIN,1);
}
